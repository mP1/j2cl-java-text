...
public final class DecimalFormatProviderTool extends LocaleProviderTool {
    ...
    void printLocales(final Set<String> languageTags) {
        ...
        this.indent();
        {
            for (final List<DecimalFormat> formats : languageTagToFormats.values()) {
                ...
                this.line(type(walkingkooka.j2cl.java.text.DecimalFormat.class) + ".register(");
                this.indent();
                {
    ...
    private void line(final DecimalFormat format,
        ...
        if (format.isParseIntegerOnly()) {
            parse |= walkingkooka.j2cl.java.text.DecimalFormat.PARSE_INTEGER_ONLY;
        }
        if (format.isParseBigDecimal()) {
            parse |= walkingkooka.j2cl.java.text.DecimalFormat.PARSE_BIG_DECIMAL;
        }
